...
import pickle
model1: RandomForestRegressor = pickle.load(open("RandomForestRegressor.pb", "rb"))
model2: AdaBoostRegressor = pickle.load(open("AdaBoostRegressor.pb", "rb"))

def predict_model1(X):
    response = model1.predict(X)
    print("Response from model1 is", response)
    return response
