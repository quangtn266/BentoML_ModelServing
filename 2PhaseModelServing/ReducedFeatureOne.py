...
print("Origial data dimension:", X.shape)

phase_one_model = LogisticRegression(random_state=0, max_iter=1000).fit(X, y)
score = phase_one_model.score(X, y)
print("accuracy of the phase 2 models: ", score)
p = pickle.dumps(phase_one_model)
print("Size of the phase 2 model in bytes:", sys.getsizeof(p))

X_new = SelectKBest(chi2, k=1).fit_transform(X, y)
print("Data dimension after feature reduction: ", X_new.shape)
